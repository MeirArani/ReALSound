...
\DeclareFloatingEnvironment[name=Code Sample]{code}
\DeclareFloatingEnvironment[name=Pseudocode]{pseudo}
...
\definecolor{codered}{rgb}{0.9,0,0.1}
\definecolor{backcolour}{rgb}{0.95,0.95,0.92}
\definecolor{brightblue}{rgb}{0,0.75,0.75}
\lstset{
    ...
    showtabs=false,
    escapechar={$}, 
    otherkeywords={touches, VERB, play, close to, warn, dies, lower},
    morekeywords={touches, VERB, play, close, to, warn, dies, lower},
...
\begin{figure}
    \begin{adjustbox}
        ...
        {
            \caption{A simple graphical overview of \rs}
            \label{fig:rsOver}
            \end{minipage}
        }, 
        rotate=270, center
        }
        \includegraphics[scale=0.85]{rsOverview.png}
    \end{adjustbox}
\end{figure}
...
\begin{figure}
    \begin{adjustbox}
        ...
        {
            \caption{An extention of \cref{fig:rsOver} including \state{Layers}}
            \label{fig:rsExt}
            \end{minipage}
        }, 
        rotate=270, center
        }
        \includegraphics[scale=0.77]{rsExtend.png}
    \end{adjustbox}
\end{figure}
...
\begin{figure}
    \begin{adjustbox}
        ...
        {
            \caption{An full overview of \rs}
            \label{fig:rsViz}
            \end{minipage}
        }, 
        rotate=270, center
        }
        \includegraphics[scale=0.7]{rsVisual.png}
    \end{adjustbox}
\end{figure}
...
To put things into programming terms, we could imagine the simplest \design implementation being a series of \texttt{if-else} statements which compare the provided entity data against relevant conditions. \cref{pseudo:design_code_examples} demonstrates three simple examples using the \texttt{python} language. The first example echoes one used in describing the \decision layer---playing a sound effect when Mario collides with a mushroom. 
...
\end{codeblock}
    \subcaption{Approaching Pitfall}
    \end{subfigure}
...
\end{codeblock}
    ...
    \captionof{pseudo}{Three samples of rudimentary \design logic.}
    \label{pseudo:design_code_examples}
\end{figure}
...
\begin{figure}[h]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \begin{codeblock}
$\color{codegreen}mario$ touches $\color{codegreen}mushroom$: play $\color{codegreen}mushroom$ $\color{codered}touch$;
\end{codeblock}
        \subcaption{On Powerup}
    \end{subfigure}
...
\end{codeblock}
    ...
    \captionof{pseudo}{Three samples of a hypothetical DSL used to \design a \emph{Super Mario Brothers} implementation. Entities are highlighted in {\color{codegreen}green}, DSL keywords are {\color{codepurple}} purple, and SFXs are rendered in {\color{codered} red.}}
    \label{pseudo:marioDSL}
\end{figure}


We can plainly see the utility of DSLs when combined with \rs's \design process. An \imp interested in a wide variety of target games could build a generalized DSL, while a different \imp could just as easily build a specific DSL to streamline \design for a particularly large and complex game. \cref{pseudo:marioDSL} provides some examples of a hypothetical DSL specifically written for \emph{Super Mario Brothers}. Much of the previous code has been reduced into far friendlier (both for reader and coder) format. 
...
\begin{figure}
    ...
    \begin{subfigure}{\textwidth}
        \centering
\begin{codeblock}
$\color{codegreen}ENTITY$ VERB $\color{codegreen}ENTITY$: VERB [$\color{codegreen}ENTITY$ | $\color{blue}ATTRIBUTE$] [$\color{codered}SFX$];
\end{codeblock}
        \subcaption{Grammar 1}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
        \centering
\begin{codeblock}
$\color{codegreen}ENTITY$ VERB: VERB ($\color{codegreen}ENTITY$ | $\color{blue}ATTRIBUTE$);
\end{codeblock}        
        \subcaption{Grammar 2}
    \end{subfigure}
    \captionof{pseudo}{Sample DSL grammar}
    \label{pseudo:dslEx}
\end{figure}
...
\begin{figure}
    \renewcommand{\tabularxcolumn}[1]{m{#1}}
    \begin{tabularx}{\linewidth}{|l|R|l|l|S{X}|}
        \hline
        ...
        \\ \hline
    \end{tabularx}
    \captionof{table}{Summary of Game \state{states}}
...
\begin{figure}
    ...
    \end{tabularx}
    \captionof{table}{Summary of general \state{Entity} Attributes}
    \label{tab:entAttr}
\end{figure}
...
\begin{figure}
    ...
    \begin{center}
        ...
        \end{tabularx}
    \end{center}
    \captionof{table}{Summary of \emph{Pong}'s \state{entities}: the \pad and \ball}
    \label{tab:paddle}
\end{figure}
...
return result
\end{codeblock}
        \captionof{code}{Abbreviated \texttt{detect} algorithm}
        \label{code:detect}
\end{figure}
...
return results
\end{codeblock}
        \captionof{code}{Abbreviated \texttt{classify} algorithm}
        \label{code:classify}
\end{figure}
...
\begin{figure}
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[height=0.4\textheight]{hitPrev.png}
        \subcaption{Frame before Player 2's \pad hits the \ball}
    \end{subfigure}
    \\[1.5em]
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[height=0.4\textheight]{hit.png}
        \subcaption{Frame of impact. Notice how object tracking for both Player 2's \pad and the \ball is lost this frame (as their corners overlap)}
    \end{subfigure}
    \caption{Example of sudden continuity loss during a \state{hit}---which may be used to detect the event}  
\label{fig:hitDect}
\end{figure}


There are, however, other errors that are not so beneficial. These errors primarily occur whenever an object is occluded by another---which usually happens when the ball moves near the large score symbols at the top of the screen, as seen in \cref{fig:cont}. Much like when striking the \pad, the \ball corners 'merge' with the score corners, creating a frame or two of bad data. 

Of course, we cannot allow this rare errors to hamper the playing experience. If a player were listening to the position of the ball, only to hear it suddenly jolt 'in-and-out of existence,' their ability to understand the game's state would be severely reduced. We can address these problems by implementing a basic sense of \tech{cross-frame persistence.} That is to say, we save the data from this frame and persist it into the next one. Using this data, we see if the changes in an objects state are sensible. For example, if an object suddenly goes from the position $(900, 540)$ to $(0, 0)$ in one frame---its almost certain that an error has occurred. If so, we re-use the previous frame's data until the object returns to sensible values in following frames. If the object continues to provide poor data, the system considers the object 'lost' and discounts it from calculations until it once again returns with reasonable data. A sample of this logic is seen in \cref{code:contDect} 

\begin{figure}
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[height=0.4\textheight]{loseBef.png}
        \subcaption{Frame before losing \ball tracking (Normal operation)}
    \end{subfigure}
    \\[1.5em]
    \begin{subfigure}{\linewidth}
        \centering
        \includegraphics[height=0.4\textheight]{loseDur.png}
        \subcaption{Frame of lost \ball continuity. Notice how the \ball overlaps corners with the score---confusing the CV algorithm}
    \end{subfigure}
    \caption{Example of tracking continuity issues with the Harris algorithm. The ball is highlighted in red for emphasis }  
\label{fig:cont}
\end{figure}
...
\end{codeblock}
    ...
    \end{subfigure}
    \captionof{code}{Decision Semantics}
    \label{code:decSem}
\end{figure}
...
def lost_frame():
    ...
    if lost_frames > MAX_LOST_FRAMES:
        deactivate()
\end{codeblock}
    \captionof{code}{Sample of object persistence code}
    \label{code:objPers}
\end{figure}
...
\begin{figure}[ht]
    \centering
    \renewcommand{\tabularxcolumn}[1]{m{#1}}
    \begin{tabularx}{\linewidth}{|M{1.3cm}|M{1cm}|S{X}|}
        \hline
        \makecell{\textbf{State}} 
        & \makecell{\textbf{Trace}}
        & \makecell{\textbf{Transition Logic}}
        \\ \hline

        \makecell{\textbf{Attract}} 
        & \makecell{\ball}
        & \makecell{\state{p1} \texttt{and} \state{p2} \trans \state{Pause}}
        \\ \hline

        \makecell{\textbf{Pause}}  
        & \makecell{\state{p1} \\ \state{p2}}
        & \makecell{\ball \texttt{and} (\state{p1} \texttt{and} \state{p2}) \trans \state{Match}}
        \\ \hline

        \makecell{\textbf{Match}}  
        & \makecell{\state{p1} \\ \state{p2} \\ \ball}
        & \makecell{\state{p1} \trans \state{p2.hit} 
        \\ \state{p2} \trans \state{p1.hit} 
        \\ \state{p1} \texttt{and} \state{p2} \trans \state{Goal} 
        \\ \ball \trans \state{Win}}
        \\ \hline
        ...
        & \makecell{\state{p1} \\ \state{p2}}
        & \makecell{
            \state{p1}.\texttt{score} $<$ 10 \texttt{and} \state{p2}.\texttt{score} $<$ 10 \trans \state{Pause} \\
            \state{p1}.\texttt{score} $>$ 10 \texttt{or} \state{p2}.\texttt{score} $>$ 10 \trans \state{Win}
            }
        \\ \hline

        \makecell[t]{\textbf{Win}}  
        & \makecell[t]{\ball}
        & \makecell[t]{\trans \state{Attract}}
        \\ \hline
    \end{tabularx}
    \captionof{table}{Summary of \state{Design} semantics}
    \label{tab:design}
\end{figure}
...
To begin, the corner data calculated by the \vision layer (\texttt{new\_corners}) is passed to the \decision layer via the \texttt{decide} function, which uses this data to update entity information and calculate the next state---as seen in \cref{code:decide}.
...
\end{codeblock}
    \captionof{code}{\decision header}
    \label{code:decide}
\end{figure}

The logic for each \state{state} (represented as unique functions) follows below \texttt{decide}. Each function returns a \state{state} function, which will execute next frame. The returned function essentially acts as the state we 'transition to' for the next frame. Take, for example, the \texttt{attract} state seen in \cref{code:attract}.
...
def attract():
    if p1.active and p2.active:
        ...
        return attract        
\end{codeblock}
    \captionof{code}{\state{Attract} logic}
    \label{code:attract}
\end{figure}

As we can see here, our \state{states} resemble the pseudocode described in previous sections. In essence, the algorithm checks for the presence of certain entities (via the \texttt{.active} variable) and makes corresponding transitions. Similar examples are seen in \cref{code:pause,code:win}.
...
\end{codeblock}
    \captionof{code}{\state{Pause} logic}
    \label{code:pause}
\end{figure}
...
def lost_frame():
    ...
    if lost_frames > MAX_LOST_FRAMES:
        deactivate()    
\end{codeblock}
        \subcaption{\texttt{lost\_frame} function}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
    \centering       
    \begin{codeblock}
...
def deactivate():
    ...
    moving = False 
\end{codeblock}
    \subcaption{\texttt{activate} and \texttt{deactivate} logic}
\end{subfigure}
...
\end{codeblock}
    \captionof{code}{Generic \state{Entity} \texttt{update\_velocity} function}
    \label{code:updateVelocity}
\end{figure}

\subsection{Special Entity Logic}
The \pad and \ball naturally have their own unique behaviors which requires additional code. Most behaviors are simple one or two-line functions that represent our entity states, as evidenced in \cref{code:padInit}.

\cref{code:ballInit} meanwhile reveals that the \ball has more complex behaviors which require additional attributes:
...
class Ball(Entity):
    ...
    def __init__(name, parent):
        ...
        pitch = Pitch.GOOD
\end{codeblock}
    \captionof{code}{\ball \texttt{init} logic}
    \label{code:ballInit}
\end{figure}
...
The \tech{update} function requires similar attention---as seen in \cref{code:ballUpdate} and supplementary code found in \cref{code:ballUpdateSup} 
...
def update(new_corners):
    ...
    if now - last_beep > beep_speed:
        beep()
\end{codeblock}
    \captionof{code}{The \ball's special \texttt{update} logic}
    \label{code:ballUpdate}
\end{figure}
...
\end{codeblock}
        \subcaption{Calculate's the \ball's current speed}
...
\end{codeblock}
        ...
        \subcaption{Calculate the \ball's current pitch}
...
class Pitch(IntEnum):
    ...
    HIGHEST = 4
\end{codeblock}
        \subcaption{Defines pitch values}
    
    \end{subfigure}

    \captionof{code}{Supplementary code for updating the \ball}
    \label{code:ballUpdateSup}
\end{figure}
...
class AudioObject(QObject):
    def __init__(parent, sound=None):
        ...
        _sound.setSize(1)
\end{codeblock}
    \captionof{code}{Definition of the \texttt{AudioObject}}
    \label{code:audObj}
\end{figure}
...
\end{codeblock}
        \subcaption{Set object position using 3D spherical data}
...
\end{codeblock}
        \subcaption{Set object position using screen-space coordinates}
...
def set_pan(pan):
    if pan is Pan.LEFT:
        ...
        set_position(az=0)
\end{codeblock}
        \subcaption{Set hard stereo panning}
    
    \end{subfigure}
    \captionof{code}{Some \texttt{AudioObject} functions}
...
\begin{figure}
    \begin{adjustbox}
        ...
        {
            \caption{Conceptualizing the \rs implementation process for \emph{Space Invaders}}
            \label{fig:impOut}
            \end{minipage}
        }, 
        rotate=270, center
        }
        \includegraphics[scale=0.45]{impOut.png}
    \end{adjustbox}
\end{figure}
